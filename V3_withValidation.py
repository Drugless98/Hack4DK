...
# Cascade (lightest → heaviest): Hash Top-10 → Embedding Top-5 → ORB Top-4 → ORB Top-2
# Final pick chosen by ORB; Ollama (heaviest) ONLY VALIDATES the final pick (yes/no). If "no", we fall back to the runner-up.
#
# Non-async version, uses at most 3 threads.
#
# Prereqs:
#   pip install pillow imagehash opencv-python-headless numpy torch open-clip-torch
#   (optional) ollama + light VLM for validation: ollama pull moondream
#
...
# Ollama (final validation with VLM) - SYNC
try:
    import ollama
...
# ------------------ Config ------------------
THREADS = 3  # at most 3 threads

# Shortlist sizes: 10 → 5 → 4 → 2
HASH_TOP  = 10   # lightest (perceptual hashes)
EMB_TOP   = 5    # embeddings (moderate)
ORB_TOP   = 4    # ORB (heavy)
FINAL_TOP = 2    # ORB refine (heavier)

HASH_SIZE = 16
TOTAL_BITS = HASH_SIZE * HASH_SIZE
IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".bmp", ".gif", ".tiff", ".webp"}
OUTPUT_TSV = "result_rot_aware_10_5_4_2.tsv"
DEBUG = True
...
def rank_by_orb_topk(qpath: Path, candidates, orb: ORBIndex, k):
    if not OPENCV_AVAILABLE:
        return candidates[:k], [(p, 0) for p in candidates[:k]]
    ...
    scored.sort(key=lambda x: x[1], reverse=True)
    return [p for p, _ in scored[:k]], scored


# ------------- Final validation via Ollama (VLM, sync) -------------
def validate_with_vlm(qpath: Path, picked: Path) -> bool:
    """
    Ask a light VLM (moondream) to validate if the picked image is the same as the query.
    Returns True (accept) or False (reject). If Ollama unavailable, returns True (skip validation).
    """
    if not (OLLAMA_AVAILABLE and picked):
        return True
    ...
    try:
        ...
        msg = {
            "role": "user",
            "content": prompt,
            "images": [str(qpath), str(picked)]
        }
        resp = ollama.chat(model="moondream", messages=[msg])
        text = resp.get("message", {}).get("content", "").strip().lower()
        if "yes" in text:
            return True
        if "no" in text:
            return False
...
def map_libraries_pipeline(lib1_dir: str, lib2_dir: str, out_tsv: str = OUTPUT_TSV):
    ...
    # Precompute for lib2 (threaded, max 3 threads)
    ...
    with open(out_tsv, "w", newline="", encoding="utf-8") as f:
        ...
        for idx, q in enumerate(qlist, 1):
            ...
            try:
                ...
                # Fast exact check via SHA map (short-circuit)
                try:
                    q_sha = sha256_file(q)
                ...
                # Step 3: Rotation-aware embeddings → Top-5
                top5 = rank_by_embeddings_topk(q, top10, k=EMB_TOP)
                if DEBUG:
                    print("  Embed Top-5:", ", ".join(p.name for p in top5))

                # Step 4: ORB (rotation robust) → Top-4
                top4, _ = rank_by_orb_topk(q, top5, orb, k=ORB_TOP)
                if DEBUG:
                    print("  ORB Top-4:", ", ".join(p.name for p in top4))

                # Step 5: ORB refine → Top-2
                top2, _ = rank_by_orb_topk(q, top4, orb, k=FINAL_TOP)
                if DEBUG:
                    print("  ORB Top-2:", ", ".join(p.name for p in top2))

                # Choose final pick by ORB inliers among Top-2
                picked = top2[0] if top2 else (top4[0] if top4 else (top5[0] if top5 else None))
                alt    = top2[1] if len(top2) > 1 else (top4[1] if len(top4) > 1 else None)

                # Ollama VALIDATION ONLY (heaviest): confirm the picked is correct; if rejected, fall back to alternate
                validation = "Skipped"
                if picked:
                    accepted = validate_with_vlm(q, picked)
                    if accepted:
                        validation = "VLM_OK" if OLLAMA_AVAILABLE else "NoVLM"
                    else:
                        if alt:
                            picked = alt
                            validation = "VLM_REJECTED_USED_ALT"
                        else:
                            validation = "VLM_REJECTED_NO_ALT"

                row = [
                    q.name,
                    ", ".join(p.name for p in top10),
                    ", ".join(p.name for p in top5),
                    ", ".join(p.name for p in top4),
                    ", ".join(p.name for p in top2),
